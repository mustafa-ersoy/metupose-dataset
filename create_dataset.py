'''
this code is used for automatic calibration from Blender software. you need to provide
a rendered image of grid of red balls and the code will find their locations from image.
also, you need to provide balls' 3D location in Blender. because they are located as grid,
it is easy to define their location via for loop
...
def is_occluded(fincoords, person_scales, person_rcoords, obj_scales, obj_rcoords, chosenobjs, category):
	...
	#bunda final resmi olusunca herhangi bir modelin hangi joiinteri baska model veya obje ile occluded olmus kontrol et
	#pixel pixel saymak zorunda degilsin, 5 pixellik gridler seklinde tarayabilirsin.
	#veya buna da gerek yok, sectigin pixel diger modelde (252,304) pixeline denk geliyorsa o pixelin transparencysine bak
	#C modeli Byi, B modeli de Ayi occluded edebilir bu durumda occluded olmayan tek obje C mesela. dikkat et
			...
			if person!=person_cnt:
				for otherperson in range(person+1, person_cnt):
					...
					if 0 < check_x < imgwid and 0 < check_y < imghei:
						#if int(img[check_y][check_x])!=0:
						if alpha.getpixel((check_x, check_y))>180:
							vis[joint]=0

			#print('person occlusion check: ', time.time()-b1)
			b2=time.time()


			for obj in range(obj_cnt):
				#print('person: ', person, 'joint: ', joint, 'object: ', obj)
				#print(obj_rcoords[obj])
				...
				#o_img=cv2.imread(obj_path+obj_list[category[obj]]+'/'+str(chosenobjs[obj]),  cv2.COLOR_RGB2RGBA)
				

				o_img=oimgs[obj]

				alpha=o_img.split()[-1]
				#print(obj_path+obj_list[category[obj]]+'/'+str(chosenobjs[obj]))
				#o_img=cv2.imread(obj_path+obj_list[category[obj]]+'/'+str(chosenobjs[obj]), cv2.IMREAD_UNCHANGED)

				imgwid, imghei=o_img.size
				if 0 < check_x < imgwid and 0 < check_y < imghei:
					#if o_img[check_y][check_x] != 0:
					if alpha.getpixel((check_x, check_y))>220:
						vis[joint]=0
			#print('object occlusion check: ', time.time()-b2)
...
def scale(person_list, rand_scale, cat):
	#bunda joint verilerine bakarak, herbir personun scalei belirlenecek ve ayni resmin icinde benzer boyutta insanlar olmasi icin
	#en sonunda scalelari estilenecek. mesela birisi 600 pixel, digeri 400 pixel boyunda ise bu ikisini 1200 pixele fitmek icin
	#ilkini 2 kat, ikincisini 3 kat buyutecegiz
	#ayrica occlusion icin obje boyutlarina da bu function karar verebilir.
	...
	for p in person_list:
		...
		# if hei_pimg>540:
		# 	person_rcoords.append([ran(-200,960*0.6), ran(0,540/2)])
		# elif hei_pimg>0 and hei_pimg <= 540:
		ycor=[]
		...
		#ycor=[ii for ii in range(540-hei_pimg, 540, 2)]+[jj for jj in range(540, 540+hei_pimg, 8)]
		...
		person_index+=1
		#print('coords: ', person_rcoords[-1])

	avg_size=sum(person_sizes)/len(person_sizes)

	#print(person_sizes)
	#print(avg_size)
	#print('rand_scale: ', rand_scale, 'person_scales: ', person_scales)
	...
	#print('avg_size: ', avg_size, obj_scales)
	return person_scales, person_rcoords, obj_scales, obj_rcoords, chosenobjs


#print('to read functions: ', time.time()-a2)

saved_img=fnlcnt+startpoint
#print(bl_list1[0], bbox[0])
#print(bl_list1[1852][0], bbox[1852])
...
while saved_img<imnum+fnlcnt+startpoint:
	...
	#backname=back_list.pop(random.randint(0, len(back_list)))
	...
	for person in range(pnum):
		for joint in range(18):
			#print('person: ', person, 'joint: ', joint, 'personname: ', person_list[person], 'rcoords: ', person_rcoords)
		...
		finpos=[]
	#print(person_rcoords, obj_rcoords, chosenobjs, cat)
	#print('to calculate final coords: ', time.time()-a4)
	a5=time.time()
	visible, inside=is_occluded(finpos1, person_scales, person_rcoords, obj_scales, obj_rcoords, chosenobjs, cat)
	#print(visible)
	#print('visibility check: ', time.time()-a5)
	a6=time.time()
	vis_cnt=[]
	...
	if sav==1:
		...
		for o in range(len(cat)):
			...
			#print(obj_path+obj_list[cat[o]]+'/'+str(chosenobjs[o]))
			img=img.resize((int(wid*obj_scales[o]), int(hei*obj_scales[o])), Image.ANTIALIAS)
			#print('size:  ', img.size, 'obj_scales: ', obj_scales[o], int(hei*obj_scales[o]))
			back.paste(img, (obj_rcoords[o][0], obj_rcoords[o][1]), img)
		#print('\n')

		back.save(final_path+str(saved_img)+'.jpg')
		#back_list.remove(backname)
		#shutil.move(bg_path+backname, bg_path+'used/'+backname)
		#print('to save final image: ', time.time()-a3)
		#print('category and objects: ', cat, chosenobjs)
...
# csvcnt=0
# for file in os.listdir('zolder/'):
# 	if file.endswith('.csv') and file[0]=='f':
# 		csvcnt+=1
# if os.path.isfile('./final.csv'):
# 	shutil.copyfile('final.csv', 'zolder/final_'+str(csvcnt)+'.csv')
...
# 	csv_writer=csv.writer(f)
	...
	#csv_writer.writerow([prev_fnlcnt])
if deleted==1:
