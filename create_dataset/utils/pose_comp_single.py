...
#bu kod, multi person pose complexity kodundan alinarak yazildigi icin bazi anlamsiz yerleri icermektedir.
...
#con2 (configuration 2) includes all connected joints whether they are redundant or not
con2=[]
...
print('max_score, index: ', max_score, scores.index(max_score)+1)
print('min_score, index: ', min_score, scores.index(min_score)+1)
...
for sco in range(len(scores)):
    ...
    if scores[sco]>th1 or scores[sco] < th2:
        rejected.append(sco+1)

print('number of rejected: ', len(rejected))


with open('utils/rejected_single_'+str(person_number)+'.csv', 'w', newline='') as reje:
